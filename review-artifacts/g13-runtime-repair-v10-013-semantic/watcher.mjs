...
async function pollOnce(harness) {
  ...
  try {
    ...
    let producerAuthority = resolveProducerAdmissionAuthority(await io.fetchProducerAuthorityComments(), authority);
    ...
    for (const receipt of receipts.sort((a, b) => Number(a.comment_id) - Number(b.comment_id))) {
      ...
      state = await renewAtBoundary(state, identity, io);
      authority = await io.readAuthority();
      producerAuthority = resolveProducerAdmissionAuthority(await io.fetchProducerAuthorityComments(), authority);
