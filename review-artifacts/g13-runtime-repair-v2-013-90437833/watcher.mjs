...
function isAuthorizedGitHubComment(comment) {
  return comment?.issue_number === String(ISSUE) &&
    comment.author_login === OWNER_LOGIN &&
...
function validateEvent(event, authority, targets) {
  ...
  if (event.duplicates?.length || !requiredFields(fields, required) || !isAuthorizedGitHubComment(event)) return { ok: false, reason: NO_SEND };
...
function validateCardAuthorization(event, authority, card, authorization) {
  ...
  if (!cardFields || cardId !== String(authorization.card_id) ||
      !isAuthorizedGitHubComment(card) ||
...
function validateReceipt(receipt, authority, existing) {
  ...
  if (receipt.duplicates?.length || !requiredFields(fields, required) || !isAuthorizedGitHubComment(receipt)) return { ok: false, reason: NO_SEND };
  ...
  if (fields.independent_observation !== "true" || fields.durable_ack !== "true" ||
      ...
      !isAuthorizedGitHubComment(receipt.observation) ||
  ...
  if (fields.receipt_state === "CONSUMED" &&
      (existing.state !== "DELIVERED" || !/^[0-9]+$/.test(fields.ack_comment_id || "") ||
       ...
       !isAuthorizedGitHubComment(receipt.acknowledgement) ||
...
function latestAuthority(comments, marker, required) {
  const valid = comments
    ...
    .filter((comment) => comment && requiredFields(comment.fields, required) && isAuthorizedGitHubComment(comment))
...
async function persistState(state, expectedRevision, identity, storage = null) {
  ...
  const initialAcquire = expectedRevision === 0 && current.revision === 0 && !current.lease &&
...
function assertConfig(config) {
  ...
  assert.equal(config.authorization.card_id, "5724538295");
  assert.equal(config.authorization.card_marker, "CONTROL_G13_RUNTIME_REPAIR_CARD_V2");
...
function prepareInitialLease(state, timestamp, identity) {
  ...
  return { ...prepareLease(state, timestamp, identity), revision: state.revision + 1 };
}
...
async function pollOnce(harness) {
  ...
  try {
    ...
    for (const receipt of receipts.sort((a, b) => Number(a.comment_id) - Number(b.comment_id))) {
      ...
      state = await renewAtBoundary(state, identity, io);
      authority = await io.readAuthority();
      const result = key ? mergeReceipt(records, enrichedReceipt, authority, io.now()) : { outcome: NO_SEND, record: null };
      if (result.record) {
...
function authorityFixture(conversationId) {
  const meta = { issue_url: "https://api.github.com/repos/D22977/gpt-browser-bridge/issues/162", user: { login: OWNER_LOGIN }, author_association: OWNER_ASSOCIATION };
  return deriveAuthority(
    [{ id: "7001", ...meta, body: `CONTROL_GENERATION_SWITCH_V1\nnew_generation=${GENERATION}\nnew_conversation_id=${conversationId}\nnew_status=ACTIVE` }],
    [{ id: "7002", ...meta, body: `CURRENT_REHYDRATION_INDEX_V118\ncontrol_generation=${GENERATION}\ncontrol_conversation_id=${conversationId}\ncontrol_status=ACTIVE` }],
    [{ id: "7003", ...meta, body: `CURRENT_REGISTRY_INDEX_V20\ncontrol_generation=${GENERATION}\ncontrol_conversation_id=${conversationId}` }]
  );
}
...
async function selfTest() {
  ...
  const cardFields = {
    state: "AUTHORIZED_BOUNDED_RUNTIME_REPAIR_WAIT_CONSUMED_STARTED", recorded_by_role: "CONTROL", control_generation: GENERATION,
    active_control_conversation_id: authority.conversationId, repository: REPO, issue: String(ISSUE), observed_default_ref: "refs/heads/review-base/gbb-gh-01",
    observed_default_head: "816e493f22240a017ceaf757725d9e689895e870", source_runtime_ready_comment_id: "5724355577",
    source_runtime_consumed_started_comment_id: "5724112631", source_artifact_ready_comment_id: "5724438720",
    source_fresh_formal_fail_comment_id: "5724531908", source_artifact_commit: "5a9a7acee25d6b0ead4053b2e0f99d2e106a88a8"
  };
  const authorization = config.authorization;
  const cardRaw = testComment(authorization.card_id, authorization.card_marker, cardFields);
  const eventFields = {
    logical_event_id: "evt-1", issue: String(ISSUE), card_or_authority_comment: authorization.card_id, control_generation: GENERATION,
  ...
  assert.equal(prepareInitialLease({ revision: 0, lease: null }, "2026-09-18T00:00:00.000Z", identityForTest("first", "host", 1)).revision, 1);
  ...
  const observationFields = { logical_event_id: "evt-1", idempotency_key: "idem-1", actor_type: "HERDR", actor_id: "w2:p1", destination_type: "HERDR", destination_id: "w2:p1", readback_verified: "true" };
  const deliveredFields = { ...sendFields, receipt_state: "DELIVERED", independent_observation: "true", durable_ack: "true", observation_id: "obs-1", observation_kind: "INDEPENDENT_DELIVERY", observation_comment_id: "7201" };
  const consumedFields = { ...deliveredFields, receipt_state: "CONSUMED", ack_comment_id: "7202", ack_logical_event_id: "evt-1", ack_idempotency_key: "idem-1", ack_actor_type: "HERDR", ack_actor_id: "w2:p1", ack_destination_type: "HERDR", ack_destination_id: "w2:p1", ack_readback_verified: "true" };
  const sendRaw = testComment("7101", RECEIPT_MARKER, sendFields);
  const deliveredRaw = testComment("7102", RECEIPT_MARKER, deliveredFields);
  const consumedRaw = testComment("7103", RECEIPT_MARKER, consumedFields);
  const observationRaw = testComment("7201", OBSERVATION_MARKER, observationFields);
  const ackRaw = testComment("7202", ACK_MARKER, observationFields);
  const card = parseGitHubRecord(cardRaw);
  const claimed = reconcileEvent(event, {}, authority, [{ role: "HERDR", id: "w2:p1" }], "2026-09-18T00:00:00.000Z", card, authorization);
  ...
  assert.equal(reconcileEvent(parseDurableComment(testComment("7104", EVENT_MARKER, { ...eventFields, logical_event_id: "evt-2" })), { "evt-1": claimed.record }, authority, [{ role: "HERDR", id: "w2:p1" }], nowIso(), card, authorization).outcome, NO_SEND);
  const badCard = { ...card, author_login: "attacker" };
  assert.equal(validateCardAuthorization(event, authority, badCard, authorization).ok, false);
  const claimedRecord = { ...claimed.record, state: "SEND_ATTEMPTED" };
  const delivered = parseDurableComment(deliveredRaw);
  assert.equal(validateReceipt({ ...delivered, observation: parseDurableComment(observationRaw) }, authority, claimedRecord).ok, true);
  assert.equal(validateReceipt({ ...delivered, observation: { ...parseDurableComment(observationRaw), issue_number: "43" } }, authority, claimedRecord).ok, false);
  assert.equal(validateReceipt({ ...delivered, observation: null }, authority, claimedRecord).ok, false);
  const consumed = parseDurableComment(consumedRaw);
  assert.equal(validateReceipt({ ...consumed, observation: parseDurableComment(observationRaw), acknowledgement: parseDurableComment(ackRaw) }, authority, { ...claimed.record, state: "DELIVERED" }).ok, true);
  assert.equal(validateReceipt({ ...consumed, observation: parseDurableComment(observationRaw), acknowledgement: parseDurableComment(observationRaw) }, authority, { ...claimed.record, state: "DELIVERED" }).ok, false);
  ...
  const harness = {
    ...
    fetchComments: async () => [eventRaw, sendRaw, deliveredRaw, consumedRaw],
