...
function isAuthorizedGitHubComment(comment, expectedIssue = ISSUE) {
...
function validateCardAuthorization(event, authority, card, authorization) {
  ...
  if (!cardFields || cardId !== String(authorization.card_id) ||
      !isAuthorizedGitHubComment(card, ISSUE) ||
...
function validateReceipt(receipt, authority, existing) {
  ...
  if (fields.independent_observation !== "true" || fields.durable_ack !== "true" ||
      ...
      !isAuthorizedGitHubComment(receipt.observation, ISSUE) ||
  ...
  if (fields.receipt_state === "CONSUMED" &&
      (existing.state !== "DELIVERED" || !/^[0-9]+$/.test(fields.ack_comment_id || "") ||
       ...
       !isAuthorizedGitHubComment(receipt.acknowledgement, ISSUE) ||
...
function latestAuthority(comments, marker, required, expectedIssue) {
  const valid = comments
    ...
    .filter((comment) => comment && requiredFields(comment.fields, required) && isAuthorizedGitHubComment(comment, expectedIssue))
...
function authorityCommentFixture(conversationId) {
  ...
  return {
    ...
    start: [{ id: "7002", ...meta(43), body: `CURRENT_REHYDRATION_INDEX_V118\ncontrol_generation=${GENERATION}\ncontrol_conversation_id=${conversationId}\ncontrol_status=ACTIVE` }],
...
async function selfTest() {
  ...
  assert.equal(validateCardAuthorization(event, authority, { ...card, issue_number: "43" }, authorization).ok, false);
  ...
  assert.equal(validateReceipt({ ...delivered, issue_number: "43", observation: parseDurableComment(observationRaw) }, authority, claimedRecord).ok, false);
