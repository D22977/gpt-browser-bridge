...
function validateCardAuthorization(event, authority, card) {
  ...
  if (!cardFields || !isAuthorizedGitHubComment(card, ISSUE) ||
      ...
      !nonEmpty(boundedScope) || boundedScope === "unbounded" ||
...
function validateReceipt(receipt, authority, existing) {
  ...
  if (fields.receipt_state !== "SEND_ATTEMPTED" &&
      (!nonEmpty(fields.transport_producer_id) || fields.transport_producer_id === receipt.author_login ||
       !nonEmpty(fields.transport_provenance) || fields.transport_provenance === "OWNER_CONTROL_PUBLICATION")) {
    return { ok: false, reason: NO_SEND };
  }
  ...
  if (fields.receipt_state === "CONSUMED" &&
      (existing.state !== "DELIVERED" || !/^[0-9]+$/.test(fields.ack_comment_id || "") ||
       ...
       receipt.acknowledgement.fields?.transport_producer_id !== fields.transport_producer_id ||
       receipt.acknowledgement.fields?.transport_provenance !== fields.transport_provenance ||
...
function reconcileEvent(event, records, authority, targets, timestamp = nowIso(), card) {
  ...
  const authorized = checked.ok && card &&
    validateCardAuthorization(event, authority, card).ok;
...
function reconcileReceipt(receipt, existing, timestamp = nowIso()) {
  ...
  const receiptCommentId = Number(receipt.comment_id || 0);
  ...
  const receiptTime = receipt.created_at || timestamp;
  ...
  if (receiptState === "SEND_ATTEMPTED") {
    if ((receiptCommentId && eventCommentId && receiptCommentId <= eventCommentId) ||
        (receiptTime && eventTime && receiptTime < eventTime)) {
  ...
  if (receiptState === "DELIVERED" &&
    ...
    if ((receiptCommentId && eventCommentId && receiptCommentId <= eventCommentId) ||
        (obsCommentId && eventCommentId && obsCommentId <= eventCommentId) ||
        (receiptCommentId && obsCommentId && receiptCommentId < obsCommentId) ||
        (receiptTime && eventTime && receiptTime < eventTime) ||
        (obsTime && eventTime && obsTime < eventTime)) {
      return { ...existing, receipt_ids: receiptIds, state: existing.state, decision: NO_SEND, reason: "CAUSAL_ORDER_VIOLATION", updated_at: timestamp };
    }
  ...
  if (receiptState === "CONSUMED" && existing.state === "DELIVERED") {
    if ((receiptCommentId && eventCommentId && receiptCommentId <= eventCommentId) ||
        (obsCommentId && eventCommentId && obsCommentId <= eventCommentId) ||
        (ackCommentId && eventCommentId && ackCommentId <= eventCommentId) ||
        (ackCommentId && obsCommentId && ackCommentId <= obsCommentId) ||
        (receiptCommentId && ackCommentId && receiptCommentId < ackCommentId) ||
        (receiptTime && eventTime && receiptTime < eventTime) ||
        (obsTime && eventTime && obsTime < eventTime) ||
        (ackTime && eventTime && ackTime < eventTime) ||
        (ackTime && obsTime && ackTime < obsTime)) {
      return { ...existing, receipt_ids: receiptIds, state: existing.state, decision: NO_SEND, reason: "CAUSAL_ORDER_VIOLATION", updated_at: timestamp };
    }
  ...
  try {
    ...
    for (const event of events.sort((a, b) => Number(a.comment_id) - Number(b.comment_id))) {
      ...
      const result = fields.logical_event_id
        ? reconcileEvent(event, records, authority, finalTargets, io.now(), card)
...
async function selfTest() {
  ...
  const cardFields = {
    ...
    logical_event_id: "evt-1", idempotency_key: "idem-1", canary_scope: "issue162-single-event",
  ...
  const ackRaw = testComment("7203", ACK_MARKER, observationFields, { user: { login: "herdr-consumer" }, created_at: "2026-09-18T00:00:04.000Z" });
  ...
  const claimed = reconcileEvent(event, {}, authority, [{ role: "HERDR", id: "w2:p1" }], "2026-09-18T00:00:00.000Z", card);
  ...
  assert.equal(reconcileEvent(parseDurableComment(testComment("7104", EVENT_MARKER, { ...eventFields, logical_event_id: "evt-2" })), { "evt-1": claimed.record }, authority, [{ role: "HERDR", id: "w2:p1" }], nowIso(), card).outcome, NO_SEND);
  const badCard = { ...card, author_login: "attacker" };
  assert.equal(validateCardAuthorization(event, authority, badCard).ok, false);
  assert.equal(validateCardAuthorization(event, authority, { ...card, issue_number: "43" }).ok, false);
  const expiredCard = { ...card, fields: { ...card.fields, expires_at: "2020-01-01T00:00:00.000Z" } };
  assert.equal(validateCardAuthorization(event, authority, expiredCard).ok, false);
  const claimedRecord = { ...claimed.record, state: "SEND_ATTEMPTED", source_comment_id: "7100" };
  ...
  assert.equal(validateReceipt({ ...delivered, observation: samePublisherObservation }, authority, claimedRecord).ok, false);
  ...
  assert.equal(validateReceipt({ ...samePublisherDelivered, observation: parseDurableComment(observationRaw) }, authority, claimedRecord).ok, false);
  const consumed = parseDurableComment(consumedRaw);
  assert.equal(validateReceipt({ ...consumed, observation: parseDurableComment(observationRaw), acknowledgement: parseDurableComment(ackRaw) }, authority, { ...claimed.record, state: "DELIVERED", source_comment_id: "7100" }).ok, true);
  assert.equal(validateReceipt({ ...consumed, observation: parseDurableComment(observationRaw), acknowledgement: parseDurableComment(observationRaw) }, authority, { ...claimed.record, state: "DELIVERED", source_comment_id: "7100" }).ok, false);
  assert.equal(validateReceipt({ ...consumed, observation: parseDurableComment(observationRaw), acknowledgement: { ...parseDurableComment(ackRaw), author_login: OWNER_LOGIN } }, authority, { ...claimed.record, state: "DELIVERED", source_comment_id: "7100" }).ok, false);

  // M01 causal ordering and preplay / inversion negatives
  ...
  const deliveredRecord = { ...claimed.record, state: "DELIVERED", source_comment_id: "7100", seen_at: "2026-09-18T00:00:00.000Z" };
