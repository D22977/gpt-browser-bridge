...
const ISSUE = 162;
const GENERATION = "014";
const EVENT_MARKER = "GBB_G14_BIDIRECTIONAL_WAKE_EVENT_V1";
const RECEIPT_MARKER = "GBB_G14_BIDIRECTIONAL_WAKE_RECEIPT_V1";
const OBSERVATION_MARKER = "GBB_G14_BIDIRECTIONAL_WAKE_OBSERVATION_V1";
const ACK_MARKER = "GBB_G14_BIDIRECTIONAL_WAKE_ACK_V1";
...
const PRODUCER_ADMISSION_MARKER = "GBB_G14_TRANSPORT_PRODUCER_ADMISSION_AUTHORITY_V1";
const PRODUCER_ADMISSION_MARKER_FAMILY = "GBB_G14_TRANSPORT_PRODUCER_ADMISSION_AUTHORITY_V";
const PRODUCER_ADMISSION_COMMENT_ID = "5747930128";
const PRODUCER_ADMISSION_ISSUE = 81;
const PINNED_SOURCE_REGISTRY_AUTHORITY = "5747742374";
...
function validateEvent(event, authority, targets) {
  ...
  const exactTargets = fields.target_id
    ? matchingTargets.filter((target) => target.id === fields.target_id)
    : matchingTargets;
  if (exactTargets.length !== 1) return { ok: false, reason: NO_SEND };
...
function validateCardAuthorization(event, authority, card, authorization = null) {
  ...
  if (!cardFields || !isAuthorizedGitHubComment(card, ISSUE) ||
      !/^GBB_G14_(WAKE|RETURN)_OPERATIONAL_CARD_V1$/.test(card.marker) ||
...
function producerAuthorityMarker(firstLine) {
  const match = /^GBB_G14_TRANSPORT_PRODUCER_ADMISSION_AUTHORITY_V([0-9]+)$/.exec(firstLine);
  if (match) return { family: true, supported: match[1] === "1", version: Number(match[1]) };
  return {
    family: firstLine.startsWith(PRODUCER_ADMISSION_MARKER_FAMILY) ||
      firstLine.startsWith("GBB_G14_TRANSPORT_PRODUCER_ADMISSION_AUTHORITY_") ||
      firstLine.startsWith("GBB_G14_TRANSPORT_PRODUCER_ADMISSION_REVOKE") ||
      firstLine.startsWith("GBB_G14_TRANSPORT_PRODUCER_ADMISSION_SUPERSEDE"),
    supported: false,
    version: null
  };
}

function parseProducerAdmissionAuthority(comment, authority, requirePinnedV1 = true) {
  const issueUrl = String(comment?.issue_url || "");
  const issueMatch = /\/issues\/(\d+)(?:$|\?|\/)/.exec(issueUrl);
  const issueNumber = comment?.issue_number ? String(comment.issue_number) : issueMatch?.[1] || null;
  if ((requirePinnedV1 && String(comment?.id || "") !== PRODUCER_ADMISSION_COMMENT_ID) ||
      issueNumber !== String(PRODUCER_ADMISSION_ISSUE) ||
      comment?.user?.login !== OWNER_LOGIN || comment?.author_association !== OWNER_ASSOCIATION) {
    throw new Error(NO_SEND);
  }

  const lines = String(comment.body || "").replaceAll("\r", "").split("\n");
  const firstLine = (lines.shift() || "").trim();
  const marker = producerAuthorityMarker(firstLine);
  if (!marker.family || !marker.supported || firstLine !== PRODUCER_ADMISSION_MARKER) throw new Error(NO_SEND);
  ...
  for (const raw of lines) {
    const line = raw.trim();
    if (!line) continue;
    if (line === "ADMITTED_PRODUCER") {
      if (inBinding) throw new Error(NO_SEND);
      if (current) admissions.push(current);
    ...
    if (inBinding || line.startsWith("- ")) continue;
    const match = /^([A-Za-z][A-Za-z0-9_]*)=(.*)$/.exec(line);
    if (!match) throw new Error(NO_SEND);
    const target = current || header;
    if (Object.prototype.hasOwnProperty.call(target, match[1])) throw new Error(NO_SEND);
    target[match[1]] = match[2].trim();
  }
  if (current) admissions.push(current);

  const requiredHeader = ["state", "control_generation", "active_control_conversation_id", "repository", "scope_issue", "source_registry_authority", "exact_match_required", "self_declared_config_is_not_authority"];
  if (!requiredFields(header, requiredHeader) ||
      header.state !== "AUTHORIZED_DURABLE_PRODUCER_ADMISSION" ||
      header.control_generation !== authority.generation ||
      header.active_control_conversation_id !== authority.conversationId ||
      header.repository !== REPO || header.scope_issue !== String(ISSUE) ||
      !authority.comment_ids?.registry ||
      header.source_registry_authority !== authority.comment_ids.registry ||
      header.exact_match_required !== "true" || header.self_declared_config_is_not_authority !== "true") {
    throw new Error(NO_SEND);
  }

  if (admissions.length !== 2) throw new Error(NO_SEND);
  const seen = new Set();
  for (const admission of admissions) {
    if (!requiredFields(admission, ["producer_id", "github_login", "provenance", "allowed_state"]) ||
        Object.keys(admission).length !== 4 || !["DELIVERED", "CONSUMED"].includes(admission.allowed_state)) {
      throw new Error(NO_SEND);
    }
    const key = [admission.producer_id, admission.github_login, admission.provenance, admission.allowed_state].join("|");
    if (seen.has(key) || admissions.some((other) => other !== admission &&
        (other.producer_id === admission.producer_id || other.github_login === admission.github_login || other.allowed_state === admission.allowed_state))) {
      throw new Error(NO_SEND);
    }
    seen.add(key);
  }
  return { comment_id: PRODUCER_ADMISSION_COMMENT_ID, generation: header.control_generation, issue: header.scope_issue, conversation_id: header.active_control_conversation_id, source_registry_authority: header.source_registry_authority, admissions };
}

function resolveProducerAdmissionAuthority(comments, authority) {
  if (!Array.isArray(comments)) throw new Error(NO_SEND);
  const ownerComments = comments.filter((comment) => {
    const issueUrl = String(comment?.issue_url || "");
    return /\/issues\/81(?:$|\?|\/)/.test(issueUrl) && comment?.user?.login === OWNER_LOGIN &&
      comment?.author_association === OWNER_ASSOCIATION;
  }).sort((a, b) => Number(a.id) - Number(b.id));

  const candidates = ownerComments.filter((comment) => {
    const body = String(comment?.body || "").replaceAll("\r", "");
    const first = body.split("\n")[0]?.trim() || "";
    const marker = producerAuthorityMarker(first);
    if (marker.family) return true;
    if (Number(comment.id) <= Number(PRODUCER_ADMISSION_COMMENT_ID)) return false;
    const parsed = parseFields(body);
    const explicit = [parsed.fields.supersedes, parsed.fields.supersedes_comment_id, parsed.fields.revokes, parsed.fields.revokes_comment_id]
      .filter(nonEmpty)
      .some((value) => String(value).split(/\s|\//)[0] === PRODUCER_ADMISSION_COMMENT_ID);
    return explicit || (/PRODUCER_ADMISSION/.test(first) && /(REVOKE|SUPERSEDE)/.test(first));
  });

  if (candidates.length !== 1 || String(candidates[0].id) !== PRODUCER_ADMISSION_COMMENT_ID) throw new Error(NO_SEND);
  const current = parseProducerAdmissionAuthority(candidates[0], authority, true);
  if (ownerComments.some((comment) => Number(comment.id) > Number(PRODUCER_ADMISSION_COMMENT_ID) &&
      String(comment.body || "").includes(PRODUCER_ADMISSION_COMMENT_ID) &&
      /(supersedes|supersede|revokes|revoke)/i.test(String(comment.body || "")))) {
    throw new Error(NO_SEND);
  }
  return current;
}

function exactAdmission(authorityRecord, producerId, login, provenance, state, authority) {
  if (!authorityRecord || authorityRecord.generation !== authority.generation ||
      authorityRecord.issue !== String(ISSUE) || authorityRecord.conversation_id !== authority.conversationId ||
      !authority.comment_ids?.registry ||
      authorityRecord.source_registry_authority !== authority.comment_ids.registry) return null;
  const matches = authorityRecord.admissions.filter((entry) =>
    entry.producer_id === producerId && entry.github_login === login &&
    entry.provenance === provenance && entry.allowed_state === state
  );
  return matches.length === 1 ? matches[0] : null;
}

function validateReceipt(receipt, authority, existing, producerAuthority = null) {
  ...
  const obsProducer = exactAdmission(
    producerAuthority,
    fields.transport_producer_id,
    observation?.author_login,
    fields.transport_provenance,
    "DELIVERED",
    authority
  );
  ...
  if (fields.independent_observation !== "true" || fields.durable_ack !== "true" ||
      ...
      obsProducer.producer_id !== fields.transport_producer_id ||
      obsProducer.provenance !== fields.transport_provenance ||
      obsProducer.allowed_state !== "DELIVERED" ||
  ...
  if (fields.receipt_state === "CONSUMED") {
    const ackProducer = exactAdmission(
      producerAuthority,
      receipt.acknowledgement?.fields?.transport_producer_id,
      receipt.acknowledgement?.author_login,
      receipt.acknowledgement?.fields?.transport_provenance,
      "CONSUMED",
      authority
    );
    ...
    if (existing.state !== "DELIVERED" || !/^[0-9]+$/.test(fields.ack_comment_id || "") ||
        ...
        receipt.acknowledgement.fields?.transport_producer_id !== ackProducer.producer_id ||
        receipt.acknowledgement.fields?.transport_provenance !== ackProducer.provenance ||
        ackProducer.allowed_state !== "CONSUMED" ||
...
function reconcileReceipt(receipt, existing, timestamp = nowIso()) {
  ...
  if (receiptState === "DELIVERED" &&
    ...
    return {
      ...
      observation_logical_event_id: receipt.observation.fields.logical_event_id,
      observation_idempotency_key: receipt.observation.fields.idempotency_key,
      observation_producer_id: receipt.observation.fields.transport_producer_id,
      observation_provenance: receipt.observation.fields.transport_provenance,
      observation_author_login: receipt.observation.author_login,
  ...
  if (receiptState === "CONSUMED" && existing.state === "DELIVERED") {
    ...
    if (!deliveredCommentId || !ackCommentId || !consumedCommentId ||
        fields.observation_comment_id !== existing.observation_comment_id ||
        receipt.observation?.comment_id !== existing.observation_comment_id ||
        receipt.observation?.created_at !== existing.observation_at ||
        receipt.observation?.fields?.logical_event_id !== existing.observation_logical_event_id ||
        receipt.observation?.fields?.idempotency_key !== existing.observation_idempotency_key ||
        receipt.observation?.fields?.transport_producer_id !== existing.observation_producer_id ||
        receipt.observation?.fields?.transport_provenance !== existing.observation_provenance ||
        receipt.observation?.author_login !== existing.observation_author_login ||
...
function mergeReceipt(records, receipt, authority, timestamp = nowIso(), producerAuthority = null) {
  const fields = receipt.fields || receipt;
  const key = fields.logical_event_id;
  const existing = records[key];
  const checked = validateReceipt(receipt, authority, existing, producerAuthority);
  if (!checked.ok) {
...
function deriveAuthority(switchComments, startComments, registryComments) {
  const controlSwitch = latestAuthority(
    ...
    ["new_generation", "new_conversation_id"],
    88
  );
  ...
  if (new Set(generations).size !== 1 ||
      ...
      (controlSwitch.fields.new_status ?? controlSwitch.fields.state) !== "ACTIVE" ||
...
function acquireSingletonGuard(options = {}) {
  const mutexName = options.mutexName || "Local\\GBB_G14_CONTROL_DOORBELL_014_V2";
...
async function readAuthoritySnapshot(fetchComments = fetchIssueComments) {
  ...
  return {
    authority: deriveAuthority(switchComments, startComments, registryComments),
    switchComments,
    startComments,
    registryComments
  };
...
async function readCurrentAuthority(fetchComments = fetchIssueComments) {
  return (await readAuthoritySnapshot(fetchComments)).authority;
}

function sameAuthorityTuple(left, right) {
  return Boolean(left && right) && left.generation === right.generation &&
    left.conversationId === right.conversationId &&
    left.comment_ids?.control_switch === right.comment_ids?.control_switch &&
    left.comment_ids?.start === right.comment_ids?.start &&
    left.comment_ids?.registry === right.comment_ids?.registry;
}

function resolveProducerFromAuthoritySnapshot(snapshot) {
  if (!snapshot?.authority || !Array.isArray(snapshot.registryComments)) throw new Error(NO_SEND);
  const registry = latestAuthority(
    snapshot.registryComments,
    AUTHORITY_MARKERS.registry,
    ["control_generation", "control_conversation_id"],
    81
  );
  if (registry.id !== snapshot.authority.comment_ids?.registry) throw new Error(NO_SEND);
  return resolveProducerAdmissionAuthority(snapshot.registryComments, snapshot.authority);
}

function admissibleHerdrStatus(pane) {
  return pane?.agent_status === "working" ||
    (pane?.agent_status === "idle" && pane?.interactive_ready === true);
}

function promptIdentityMatches(prompted, target) {
  const paneId = prompted?.agent?.pane_id;
  const session = prompted?.agent?.agent_session?.value || null;
  return paneId === target?.id && (target?.session ? session === target.session : nonEmpty(session));
}
...
async function readHerdrTargets(config) {
  ...
  return panes
    ...
    .map((pane) => ({
      ...
      cwd: pane.cwd,
      interactive_ready: pane.interactive_ready === true
    }));
}
...
function assertState(state) {
  ...
  if (!state || state.schema !== "CONTROL_DOORBELL_G14_STATE_V1" || !Number.isInteger(state.revision) || state.revision < 0 ||
...
function assertConfig(config) {
  ...
  assert.equal(config.schema, "CONTROL_DOORBELL_G14_CONFIG_V1");
  ...
  assert.equal(config.task.name, "GBB_G13_RESIDENT");
  ...
  assert.equal(config.authorization.operational_card_prefix, "GBB_G14_");
  ...
  assert.equal(String(config.transport.producer_admission_authority?.issue), String(PRODUCER_ADMISSION_ISSUE));
  assert.equal(config.transport.producer_admission_authority?.comment_id, PRODUCER_ADMISSION_COMMENT_ID);
  assert.equal(config.transport.producer_admission_authority?.marker, PRODUCER_ADMISSION_MARKER);
  assert.equal(config.transport.producer_admission_authority?.marker_family, PRODUCER_ADMISSION_MARKER_FAMILY + "*");
  assert.equal(config.transport.producer_admission_authority?.source_registry_authority, PINNED_SOURCE_REGISTRY_AUTHORITY);
  assert.equal(Object.hasOwn(config.transport, "admitted_producers"), false);
  assert.equal(admissibleHerdrStatus({ agent_status: "idle", interactive_ready: true }), true);
  assert.equal(admissibleHerdrStatus({ agent_status: "idle", interactive_ready: false }), false);
  assert.equal(promptIdentityMatches({ agent: { pane_id: "w2:pZ", agent_session: { value: "new-session" } } }, { id: "w2:pZ", session: null }), true);
  assert.equal(promptIdentityMatches({ agent: { pane_id: "w2:pZ", agent_session: { value: "other" } } }, { id: "w2:pZ", session: "expected" }), false);
  return config;
}
...
function heartbeat(state, authority, identity, timestamp, records) {
  ...
  state.task_name = "GBB_G13_RESIDENT";
...
function productionIo() {
  return {
    ...
    readAuthoritySnapshot: () => readAuthoritySnapshot(),
...
function herdrPromptText(event, target, sendCommentId) {
  const fields = event.fields || event;
  return [
    `Read GitHub directly. This is one bounded G14 inbound wake for Issue #${ISSUE}.`,
    `source_event_comment_id=${event.comment_id}`,
    `send_attempted_receipt=${sendCommentId}`,
    `logical_event_id=${fields.logical_event_id}`,
    `idempotency_key=${fields.idempotency_key}`,
    `target_pane=${target.id}`,
    `target_session=${target.session || "CREATED_ON_PROMPT"}`,
...
async function sendHerdrOnce({ event, record, authority, card = null, config = null, io }) {
  ...
  const matches = (targets || []).filter((target) => target.role === "HERDR" && target.id === record.target_id);
  if (matches.length !== 1 || matches[0].id !== record.target_id ||
      (!nonEmpty(matches[0].session) && !matches[0].interactive_ready)) return noSend();
  ...
  try {
    ...
    if (prompted?.type !== "agent_prompted" || prompted.agent?.pane_id !== target.id) throw new Error("PROMPT_IDENTITY_MISMATCH");
    if (target.session) {
      if (!promptIdentityMatches(prompted, target)) throw new Error("PROMPT_IDENTITY_MISMATCH");
    } else {
      const refreshedTargets = await io.readTargets(config);
      const refreshed = (refreshedTargets || []).filter((candidate) => candidate.role === "HERDR" && candidate.id === target.id);
      if (refreshed.length !== 1 || !nonEmpty(refreshed[0].session)) throw new Error("PROMPT_SESSION_READBACK_MISSING");
    }
...
async function pollOnce(harness) {
  ...
  try {
    ...
    let authority = authoritySnapshot.authority;
    let producerAuthority = resolveProducerFromAuthoritySnapshot(authoritySnapshot);
    let verifiedSnapshot = await io.readAuthoritySnapshot();
    if (!sameAuthorityTuple(authority, verifiedSnapshot.authority)) throw new Error(NO_SEND);
    authoritySnapshot = verifiedSnapshot;
...
function authorityCommentFixture(conversationId) {
  ...
  return {
    ...
    registry: [{ id: PINNED_SOURCE_REGISTRY_AUTHORITY, ...meta(81), body: `CURRENT_REGISTRY_INDEX_V19\ncontrol_generation=${GENERATION}\ncontrol_conversation_id=${conversationId}` }]
  };
}
...
function producerAuthorityFixture() {
  return {
    id: PRODUCER_ADMISSION_COMMENT_ID,
    ...metaFixture(PRODUCER_ADMISSION_ISSUE),
    body: `GBB_G14_TRANSPORT_PRODUCER_ADMISSION_AUTHORITY_V1
state=AUTHORIZED_DURABLE_PRODUCER_ADMISSION
control_generation=014
active_control_conversation_id=6aa41339-7b94-83e8-9d24-24de07cf5235
repository=D22977/gpt-browser-bridge
scope_issue=162
source_registry_authority=5747742374
source_prior_formal_review=5729202105
exact_match_required=true
self_declared_config_is_not_authority=true
...
provenance=HERDR_LOCAL_OBSERVATION
allowed_state=DELIVERED

ADMITTED_PRODUCER
producer_id=herdr-consumer
github_login=herdr-consumer
provenance=HERDR_LOCAL_ACK
allowed_state=CONSUMED

BINDING
- exact tuple authority`,
    created_at: "2026-09-18T11:16:38Z"
  };
}
...
async function selfTest() {
  ...
  const producerAuthorityRaw = producerAuthorityFixture();
  const producerAuthority = parseProducerAdmissionAuthority(producerAuthorityRaw, authority);
  assert.equal(authority.comment_ids.registry, PINNED_SOURCE_REGISTRY_AUTHORITY);
  assert.equal(producerAuthority.comment_id, PRODUCER_ADMISSION_COMMENT_ID);
  assert.deepEqual(producerAuthority.admissions.map((entry) => [entry.producer_id, entry.github_login, entry.provenance, entry.allowed_state]), [
    ["herdr-daemon", "herdr-agent", "HERDR_LOCAL_OBSERVATION", "DELIVERED"],
    ["herdr-consumer", "herdr-consumer", "HERDR_LOCAL_ACK", "CONSUMED"]
  ]);
  assert.deepEqual(resolveProducerAdmissionAuthority([producerAuthorityRaw], authority), producerAuthority);
  const driftedAuthority = { ...authority, comment_ids: { ...authority.comment_ids, registry: "7003" } };
  assert.throws(() => parseProducerAdmissionAuthority(producerAuthorityRaw, driftedAuthority), /CONTROL_REQUIRED_NO_SEND/);
  assert.equal(exactAdmission(producerAuthority, "herdr-daemon", "herdr-agent", "HERDR_LOCAL_OBSERVATION", "DELIVERED", driftedAuthority), null);
  assert.throws(() => resolveProducerAdmissionAuthority([producerAuthorityRaw], { ...authority, comment_ids: { ...authority.comment_ids, registry: null } }), /CONTROL_REQUIRED_NO_SEND/);
  assert.throws(() => resolveProducerAdmissionAuthority([], authority), /CONTROL_REQUIRED_NO_SEND/);
  assert.throws(() => resolveProducerAdmissionAuthority([producerAuthorityRaw, { ...producerAuthorityRaw, id: "5747930130" }], authority), /CONTROL_REQUIRED_NO_SEND/);
  const duplicateAdmissionRaw = { ...producerAuthorityRaw, body: producerAuthorityRaw.body.replace("producer_id=herdr-daemon", "producer_id=herdr-daemon\nproducer_id=forged") };
  assert.throws(() => parseProducerAdmissionAuthority(duplicateAdmissionRaw, authority), /CONTROL_REQUIRED_NO_SEND/);
  const conflictingAdmissionRaw = { ...producerAuthorityRaw, body: producerAuthorityRaw.body.replace("producer_id=herdr-consumer", "producer_id=herdr-daemon") };
  assert.throws(() => parseProducerAdmissionAuthority(conflictingAdmissionRaw, authority), /CONTROL_REQUIRED_NO_SEND/);
  const malformedAdmissionRaw = { ...producerAuthorityRaw, body: producerAuthorityRaw.body.replace("allowed_state=CONSUMED", "") };
  assert.throws(() => parseProducerAdmissionAuthority(malformedAdmissionRaw, authority), /CONTROL_REQUIRED_NO_SEND/);
  const newerSupported = { ...producerAuthorityRaw, id: "5747930131", body: producerAuthorityRaw.body.replace(PRODUCER_ADMISSION_MARKER, "GBB_G14_TRANSPORT_PRODUCER_ADMISSION_AUTHORITY_V1") };
  assert.throws(() => resolveProducerAdmissionAuthority([producerAuthorityRaw, newerSupported], authority), /CONTROL_REQUIRED_NO_SEND/);
  const unsupportedNewer = { ...producerAuthorityRaw, id: "5747930132", body: producerAuthorityRaw.body.replace(PRODUCER_ADMISSION_MARKER, "GBB_G14_TRANSPORT_PRODUCER_ADMISSION_AUTHORITY_V2") };
  assert.throws(() => resolveProducerAdmissionAuthority([producerAuthorityRaw, unsupportedNewer], authority), /CONTROL_REQUIRED_NO_SEND/);
  const malformedNewer = { ...producerAuthorityRaw, id: "5747930133", body: producerAuthorityRaw.body.replace(PRODUCER_ADMISSION_MARKER, "GBB_G14_TRANSPORT_PRODUCER_ADMISSION_AUTHORITY_V2").replace("scope_issue=162", "") };
  assert.throws(() => resolveProducerAdmissionAuthority([producerAuthorityRaw, malformedNewer], authority), /CONTROL_REQUIRED_NO_SEND/);
  const competingNewer = { ...producerAuthorityRaw, id: "5747930134", body: producerAuthorityRaw.body.replace(PRODUCER_ADMISSION_MARKER, "GBB_G14_TRANSPORT_PRODUCER_AUTHORITY_V3") };
  assert.throws(() => resolveProducerAdmissionAuthority([producerAuthorityRaw, unsupportedNewer, competingNewer], authority), /CONTROL_REQUIRED_NO_SEND/);
  const explicitRevoke = { id: "5747930135", ...metaFixture(81), body: `GBB_G14_TRANSPORT_PRODUCER_ADMISSION_REVOKE_V1\nrevokes_comment_id=${PRODUCER_ADMISSION_COMMENT_ID}` };
  assert.throws(() => resolveProducerAdmissionAuthority([producerAuthorityRaw, explicitRevoke], authority), /CONTROL_REQUIRED_NO_SEND/);
  const explicitSuperseder = { id: "5747930136", ...metaFixture(81), body: `GBB_G14_TRANSPORT_PRODUCER_AUTHORITY_UPDATE_V1\nsupersedes=${PRODUCER_ADMISSION_COMMENT_ID}` };
  assert.throws(() => resolveProducerAdmissionAuthority([producerAuthorityRaw, explicitSuperseder], authority), /CONTROL_REQUIRED_NO_SEND/);
  assert.throws(() => resolveProducerAdmissionAuthority(null, authority), /CONTROL_REQUIRED_NO_SEND/);
  ...
  assert.deepEqual(resolveProducerFromAuthoritySnapshot(coherentSnapshot), producerAuthority);
  const newerRegistry = { id: "5747930141", ...metaFixture(81), body: `CURRENT_REGISTRY_INDEX_V20\ncontrol_generation=${GENERATION}\ncontrol_conversation_id=${authority.conversationId}` };
  assert.throws(() => resolveProducerFromAuthoritySnapshot({ ...coherentSnapshot, registryComments: [...coherentSnapshot.registryComments, newerRegistry] }), /CONTROL_REQUIRED_NO_SEND/);
  assert.throws(() => resolveProducerFromAuthoritySnapshot({ ...coherentSnapshot, registryComments: [producerAuthorityRaw] }), /CONTROL_REQUIRED_NO_SEND/);
  const malformedRegistry = { id: "5747930141", ...metaFixture(81), body: `CURRENT_REGISTRY_INDEX_V20\ncontrol_generation=${GENERATION}` };
  assert.throws(() => resolveProducerFromAuthoritySnapshot({ ...coherentSnapshot, registryComments: [...coherentSnapshot.registryComments, malformedRegistry] }), /CONTROL_REQUIRED_NO_SEND/);
  const duplicateRegistry = { id: "5747930141", ...metaFixture(81), body: `CURRENT_REGISTRY_INDEX_V20\ncontrol_generation=${GENERATION}\ncontrol_generation=${GENERATION}\ncontrol_conversation_id=${authority.conversationId}` };
  assert.throws(() => resolveProducerFromAuthoritySnapshot({ ...coherentSnapshot, registryComments: [...coherentSnapshot.registryComments, duplicateRegistry] }), /CONTROL_REQUIRED_NO_SEND/);
  assert.equal(sameAuthorityTuple(authority, { ...authority, comment_ids: { ...authority.comment_ids, registry: "5645734141" } }), false);
  ...
  const cardRaw = testComment("5725999001", "GBB_G14_WAKE_OPERATIONAL_CARD_V1", cardFields);
  const eventFields = {
  ...
  assert.equal(validateEvent(event, authority, [{ role: "HERDR", id: "w2:p1" }, { role: "HERDR", id: "w2:pX" }]).ok, true);
  ...
  let staleLeaseState = {
    schema: "CONTROL_DOORBELL_G14_STATE_V1", revision: 1, records: {},
  ...
  const parsedObservation = parseDurableComment(observationRaw);
  const parsedAck = parseDurableComment(ackRaw);
  assert.equal(validateReceipt({ ...delivered, observation: parsedObservation }, authority, claimedRecord, producerAuthority).ok, true);
  assert.equal(validateReceipt({ ...delivered, issue_number: "43", observation: parsedObservation }, authority, claimedRecord, producerAuthority).ok, false);
  assert.equal(validateReceipt({ ...delivered, observation: { ...parsedObservation, issue_number: "43" } }, authority, claimedRecord, producerAuthority).ok, false);
  assert.equal(validateReceipt({ ...delivered, observation: null }, authority, claimedRecord, producerAuthority).ok, false);
  const samePublisherObservation = { ...parsedObservation, author_login: OWNER_LOGIN };
  assert.equal(validateReceipt({ ...delivered, observation: samePublisherObservation }, authority, claimedRecord, producerAuthority).ok, false);
  const forgedProducerObservation = { ...parsedObservation, fields: { ...parsedObservation.fields, transport_producer_id: "forged-producer" } };
  const forgedProducerDelivered = { ...delivered, fields: { ...delivered.fields, transport_producer_id: "forged-producer" } };
  assert.equal(validateReceipt({ ...forgedProducerDelivered, observation: forgedProducerObservation }, authority, claimedRecord, producerAuthority).ok, false);
  const forgedProvenanceObservation = { ...parsedObservation, fields: { ...parsedObservation.fields, transport_provenance: "FORGED_PROVENANCE" } };
  const forgedProvenanceDelivered = { ...delivered, fields: { ...delivered.fields, transport_provenance: "FORGED_PROVENANCE" } };
  assert.equal(validateReceipt({ ...forgedProvenanceDelivered, observation: forgedProvenanceObservation }, authority, claimedRecord, producerAuthority).ok, false);
  const unrelatedCopiedObservation = { ...parsedObservation, author_login: "unrelated-account" };
  assert.equal(validateReceipt({ ...delivered, observation: unrelatedCopiedObservation }, authority, claimedRecord, producerAuthority).ok, false);
  const consumed = parseDurableComment(consumedRaw);
  const deliveredRecord = { ...reconcileReceipt({ ...delivered, observation: parsedObservation }, claimedRecord, "2026-09-18T00:00:03.000Z") };
  assert.equal(validateReceipt({ ...consumed, observation: parsedObservation, acknowledgement: parsedAck }, authority, deliveredRecord, producerAuthority).ok, true);
  assert.equal(validateReceipt({ ...consumed, observation: parsedObservation, acknowledgement: parsedObservation }, authority, deliveredRecord, producerAuthority).ok, false);
  assert.equal(validateReceipt({ ...consumed, observation: parsedObservation, acknowledgement: { ...parsedAck, author_login: OWNER_LOGIN } }, authority, deliveredRecord, producerAuthority).ok, false);
  const forgedAck = { ...parsedAck, author_login: "unrelated-account" };
  assert.equal(validateReceipt({ ...consumed, observation: parsedObservation, acknowledgement: forgedAck }, authority, deliveredRecord, producerAuthority).ok, false);
  const forgedAckProducer = { ...parsedAck, fields: { ...parsedAck.fields, transport_producer_id: "forged-consumer" } };
  assert.equal(validateReceipt({ ...consumed, observation: parsedObservation, acknowledgement: forgedAckProducer }, authority, deliveredRecord, producerAuthority).ok, false);
  const forgedAckProvenance = { ...parsedAck, fields: { ...parsedAck.fields, transport_provenance: "FORGED_ACK" } };
  assert.equal(validateReceipt({ ...consumed, observation: parsedObservation, acknowledgement: forgedAckProvenance }, authority, deliveredRecord, producerAuthority).ok, false);

  const observationB = { ...parsedObservation, comment_id: "7205", created_at: "2026-09-18T00:00:03.500Z" };
  const consumedWithObservationB = { ...consumed, fields: { ...consumed.fields, observation_comment_id: "7205" }, observation: observationB, acknowledgement: parsedAck };
  assert.equal(reconcileReceipt(consumedWithObservationB, deliveredRecord).reason, "CAUSAL_ORDER_VIOLATION");

  const preplayedDelivered = { ...delivered, observation: parsedObservation };
  ...
  const registryV20 = { id: "5747930141", ...metaFixture(81), body: `CURRENT_REGISTRY_INDEX_V20\ncontrol_generation=${GENERATION}\ncontrol_conversation_id=${authority.conversationId}` };
  const authorityV20 = { ...authority, comment_ids: { ...authority.comment_ids, registry: "5747930141" } };
  const producerV20 = { ...producerAuthorityRaw, body: producerAuthorityRaw.body.replace("source_registry_authority=5747742374", "source_registry_authority=5747742375") };
  const snapshotV20 = {
  ...
  const producerRevoke = { id: "5747930132", ...metaFixture(81), body: `GBB_G14_TRANSPORT_PRODUCER_ADMISSION_REVOKE_V1\nrevokes_comment_id=${PRODUCER_ADMISSION_COMMENT_ID}` };
  ...
  const malformedRegistrySnapshot = coherentSnapshotFor(authority, [{ id: "5747930141", ...metaFixture(81), body: `CURRENT_REGISTRY_INDEX_V20\ncontrol_generation=${GENERATION}` }]);
  await runSequencedFailure([snapshotV19, snapshotV19, snapshotV19, snapshotV19, malformedRegistrySnapshot]);
  const duplicateRegistrySnapshot = coherentSnapshotFor(authority, [{ id: "5747930141", ...metaFixture(81), body: `CURRENT_REGISTRY_INDEX_V20\ncontrol_generation=${GENERATION}\ncontrol_generation=${GENERATION}\ncontrol_conversation_id=${authority.conversationId}` }]);
  ...
  const mutexName = `Local\\GBB_G14_SELFTEST_${process.pid}_${Date.now()}`;
