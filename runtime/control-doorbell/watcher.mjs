...
const PRODUCER_ADMISSION_MARKER = "GBB_G13_TRANSPORT_PRODUCER_ADMISSION_AUTHORITY_V1";
const PRODUCER_ADMISSION_MARKER_FAMILY = "GBB_G13_TRANSPORT_PRODUCER_ADMISSION_AUTHORITY_V";
const PRODUCER_ADMISSION_COMMENT_ID = "5729213249";
const PRODUCER_ADMISSION_ISSUE = 81;
const PINNED_SOURCE_REGISTRY_AUTHORITY = "5645734140";
...
function producerAuthorityMarker(firstLine) {
  const match = /^GBB_G13_TRANSPORT_PRODUCER_ADMISSION_AUTHORITY_V([0-9]+)$/.exec(firstLine);
  ...
  return {
    family: firstLine.startsWith(PRODUCER_ADMISSION_MARKER_FAMILY) ||
      firstLine.startsWith("GBB_G13_TRANSPORT_PRODUCER_ADMISSION_AUTHORITY_") ||
      firstLine.startsWith("GBB_G13_TRANSPORT_PRODUCER_ADMISSION_REVOKE") ||
      firstLine.startsWith("GBB_G13_TRANSPORT_PRODUCER_ADMISSION_SUPERSEDE"),
    supported: false,
    version: null
  };
}
...
function producerAuthorityFixture() {
  return {
    ...
    body: `GBB_G13_TRANSPORT_PRODUCER_ADMISSION_AUTHORITY_V1
...
scope_issue=162
source_registry_authority=5645734140
...
async function selfTest() {
  ...
  assert.throws(() => resolveProducerAdmissionAuthority([producerAuthorityRaw, { ...producerAuthorityRaw, id: "5729213250" }], authority), /CONTROL_REQUIRED_NO_SEND/);
  ...
  const newerSupported = { ...producerAuthorityRaw, id: "5729213251", body: producerAuthorityRaw.body.replace(PRODUCER_ADMISSION_MARKER, "GBB_G13_TRANSPORT_PRODUCER_ADMISSION_AUTHORITY_V1") };
  assert.throws(() => resolveProducerAdmissionAuthority([producerAuthorityRaw, newerSupported], authority), /CONTROL_REQUIRED_NO_SEND/);
  const unsupportedNewer = { ...producerAuthorityRaw, id: "5729213252", body: producerAuthorityRaw.body.replace(PRODUCER_ADMISSION_MARKER, "GBB_G13_TRANSPORT_PRODUCER_ADMISSION_AUTHORITY_V2") };
  assert.throws(() => resolveProducerAdmissionAuthority([producerAuthorityRaw, unsupportedNewer], authority), /CONTROL_REQUIRED_NO_SEND/);
  const malformedNewer = { ...producerAuthorityRaw, id: "5729213253", body: producerAuthorityRaw.body.replace(PRODUCER_ADMISSION_MARKER, "GBB_G13_TRANSPORT_PRODUCER_ADMISSION_AUTHORITY_V2").replace("scope_issue=162", "") };
  assert.throws(() => resolveProducerAdmissionAuthority([producerAuthorityRaw, malformedNewer], authority), /CONTROL_REQUIRED_NO_SEND/);
  const competingNewer = { ...producerAuthorityRaw, id: "5729213254", body: producerAuthorityRaw.body.replace(PRODUCER_ADMISSION_MARKER, "GBB_G13_TRANSPORT_PRODUCER_ADMISSION_AUTHORITY_V3") };
  ...
  const explicitRevoke = { id: "5729213255", ...metaFixture(81), body: `GBB_G13_TRANSPORT_PRODUCER_ADMISSION_REVOKE_V1\nrevokes_comment_id=${PRODUCER_ADMISSION_COMMENT_ID}` };
  assert.throws(() => resolveProducerAdmissionAuthority([producerAuthorityRaw, explicitRevoke], authority), /CONTROL_REQUIRED_NO_SEND/);
  const explicitSuperseder = { id: "5729213256", ...metaFixture(81), body: `GBB_G13_TRANSPORT_PRODUCER_AUTHORITY_UPDATE_V1\nsupersedes=${PRODUCER_ADMISSION_COMMENT_ID}` };
  ...
  const newerRegistry = { id: "5645734141", ...metaFixture(81), body: `CURRENT_REGISTRY_INDEX_V20\ncontrol_generation=${GENERATION}\ncontrol_conversation_id=${authority.conversationId}` };
  ...
  const malformedRegistry = { id: "5645734141", ...metaFixture(81), body: `CURRENT_REGISTRY_INDEX_V20\ncontrol_generation=${GENERATION}` };
  ...
  const duplicateRegistry = { id: "5645734141", ...metaFixture(81), body: `CURRENT_REGISTRY_INDEX_V20\ncontrol_generation=${GENERATION}\ncontrol_generation=${GENERATION}\ncontrol_conversation_id=${authority.conversationId}` };
  ...
  const snapshotC = {
    authority: authorityV20,
    switchComments: [{ ...authorityComments.controlSwitch[0], id: "9999999999" }],
    startComments: authorityComments.start,
    registryComments: [...authorityComments.registry, registryV20, producerAuthorityRaw]
  };
